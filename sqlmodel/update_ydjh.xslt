...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:cim="http://iec.ch/TC57/2003/CIM-schema-cim10#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    ...
    <xsl:template match="/Datas">
         <xsl:for-each select="ITEM">
         	  ...
         	  <xsl:if test="not(string(JXJHYF) eq '')">,JXJHYF=</xsl:if><xsl:if test="not(string(JXJHYF) eq '')"><xsl:value-of select='concat("to_date(&apos;",JXJHYF,"&apos;,&apos;YYYY-MM-DD HH24:Mi:SS&apos;)")'/></xsl:if>
         	  ...
         	  <xsl:for-each select="*">
         	  	 <xsl:if test="not(string(node()) eq '') and name()!='OBJ_ID' and name()!='GZWCSJ' and name()!='JHBGSJ' and name()!='JHKGSJ' and name()!='JHQXSJ' and name()!='JHWGSJ' and name()!='JXJHYF' and name()!='PZWGSJ' and name()!='SBSJ' and name()!='SYSTEMDATE' and name()!='PZKGSJ'">,<xsl:value-of select="name()"/>='<xsl:value-of select="."/>'
         	   	 </xsl:if>
